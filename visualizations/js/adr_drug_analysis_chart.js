/**
 * ADR Drug Analysis Chart Module (WHO Drug Names)
 * Comprehensive analysis of drugs involved in adverse drug reactions
 */

// Ensure namespace exists
window.ADRCharts = window.ADRCharts || {};

// Drug analysis chart module
ADRCharts.drugAnalysis = {
    // Private properties
    ...
    _colors: {
        ...
        bar: 'rgba(52, 152, 219, 0.8)'
    },
    
    /**
     * Process data for drug analysis
    ...
    processData: function(data, analysisMode = 'cases') {
        ...
        data.forEach((row) => {
            const drugs = ADRCharts.utils.parseDrugNames(row['Drug name (WHODrug)']);
        ...
        const config = {
            ...
            options: {
                ...
                plugins: {
                    title: {
                        display: true,
                        text: `Top Drugs by ${processed.analysisMode === 'cases' ? 'Cases' : 'Mentions'}`,
                        font: {
                            size: 16
                        }
                    },
        ...
        const config = {
            ...
            options: {
                ...
                plugins: {
                    title: {
                        display: true,
                        text: `Top 20 Drugs by ${processed.analysisMode === 'cases' ? 'Cases' : 'Mentions'}`,
                        font: {
                            size: 16
                        }
                    },
        ...
        const config = {
            ...
            options: {
                ...
                plugins: {
                    ...
                    title: {
                        display: true,
                        text: `All Drugs - ${processed.analysisMode === 'cases' ? 'Cases' : 'Mentions'} Analysis`,
                        font: {
                            size: 16
                        }
                    }
    ...
    update: function(newData, options) {
        ...
        if (viewType === 'overview') {
            if (this._pieChartInstance) {
                ...
                this._pieChartInstance.options.plugins.title.text = `Top Drugs by ${analysisMode === 'cases' ? 'Cases' : 'Mentions'}`;
            ...
            if (this._barChartInstance) {
                ...
                this._barChartInstance.options.plugins.title.text = `Top 20 Drugs by ${analysisMode === 'cases' ? 'Cases' : 'Mentions'}`;
            ...
            this._detailedBarChartInstance.options.plugins.title.text = `All Drugs - ${analysisMode === 'cases' ? 'Cases' : 'Mentions'} Analysis`;
    ...
    exportData: function(data, filename, analysisMode = 'cases') {
        ...
        // Add summary
        csvData.push({});
        csvData.push({ 'Drug Name (WHO)': 'SUMMARY', 'Number of Cases': '' });
        csvData.push({ 'Drug Name (WHO)': 'Total Unique Drugs', 'Number of Cases': Object.keys(drugData).length });
        csvData.push({ 'Drug Name (WHO)': 'Total Cases with Drugs', 'Number of Cases': processed.totalCasesWithDrugs });
        csvData.push({ 'Drug Name (WHO)': 'Cases with Single Drug', 'Number of Cases': processed.casesWithSingleDrug });
        csvData.push({ 'Drug Name (WHO)': 'Cases with Multiple Drugs', 'Number of Cases': processed.casesWithMultipleDrugs });
        
        ADRCharts.utils.exportToCSV(csvData, filename || 'adr_drug_analysis_who.csv');
    },
